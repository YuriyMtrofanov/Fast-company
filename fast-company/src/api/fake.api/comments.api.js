const comments = [
    {
        _id: "67rdca3eeb7f6fg",
        userId: "67rdca3eeb7f6fgeed471815", // id автора комментария
        pageId: "67rdca3eeb7f6fgeed471815", // id пользователя, на которой помещен комментарий
...
// комментарии на странице пользователя с id = userId
...
// метод, который записывает комментарий и присваивает ему дату
const add = (data) =>
    new Promise((resolve) => {
        window.setTimeout(function () {
...
// метод удаления комментария. id - это id комментария
const remove = (id) =>
    new Promise((resolve) => {
        window.setTimeout(function () {
            const comments = JSON.parse(localStorage.getItem("comments"));
